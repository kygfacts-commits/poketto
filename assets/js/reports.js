...
export function buildTopCategories(transactions, categoriesById, start, end, n = 5) {
  const txs = transactions.filter((tx) => tx.date >= start && tx.date <= end && tx.type === 'expense');
...
export function buildBudgetSummary(budgets, transactions, start, end) {
  ...
  const totalUsed = transactions
    .filter((tx) => tx.date >= start && tx.date <= end && tx.type === 'expense' && budgetCatIds.has(tx.category_id))
