...
// Shell inti yang diprecache saat install (semua same-origin, sudah diverifikasi ada).
const PRECACHE = [
  '/index.html',
  '/pages/home.html',
  '/pages/login.html',
...
self.addEventListener('fetch', (event) => {
  ...
  // (1) Cross-origin (Supabase API, CDN Tailwind/jsDelivr, Google Fonts) → biarkan
  //     browser fetch langsung TANPA cache. Ini yang menjamin data Supabase selalu segar.
  if (url.origin !== self.location.origin) return;

  // (2) Defense-in-depth: andai suatu hari Supabase diproxy same-origin, tetap tolak cache.
  if (url.hostname.endsWith('supabase.co')) return;

  // (3) Navigasi halaman → NETWORK-FIRST (shell fresh), fallback ke cache saat offline.
  if (req.mode === 'navigate') {
    event.respondWith(networkFirst(req));
    return;
  }

  // (4) Asset statis same-origin → CACHE-FIRST.
